...
topLevelInfer expr = do
  ...
  table        <- DT.trace ("cons:   " ++ show cons ++ "\ndiss:   " ++ show diss) $ constructTable cons diss
...
infer (Letrec b) = do
  ...
  -- θ = solve(τ1 <: τ')
  table <- DT.trace ("cons in let:    " ++ show (initC' ((t1, t'):sloc))) $ constructTable cons1 dis1
...
destruct ((_,(_, t2)):lqc) table seen | topLikeP t2 = destruct lqc table seen
-- BCD distributivity Left - Right
destruct ((EmptyQ, (t1, In (Inl (And t2 t3)))):lqc) table seen | t1 == t2 || t1 == t3 = destruct lqc table seen
...
-- take from queue
destruct (c@(QA p q,(In (Inl (Arr a1 a2)),   a)) :lqc) table seen = destruct ((EmptyQ,(p, a1)):(q,(a2, a)):lqc) table (c:seen)
...
destruct (c@(q,(In (Inl (And a1 a2)), a3))  :lqc) table seen = case destruct ((q, (a1, a3)):lqc) table (c:seen) of
  Just tbl1 -> case destruct ((q, (a2, a3)):lqc) table (c:seen) of
    Just tbl2 -> case And a1 a2 of
      And (In (Inr (Uni u1))) (In (Inr (Uni u2))) -> destruct lqc table (c:seen)
      _ -> destruct lqc (mergeTables table tbl1 tbl2) ((q, (a1, a3)):(q, (a2, a3)):seen)
    Nothing   -> Just tbl1
  Nothing -> case destruct ((q, (a2, a3)):lqc) table (c:seen) of
    Just tbl2 -> Just tbl2
    Nothing   -> Nothing
destruct (c@(QA p q,((In (Inr (Uni u))),a)):lqc) table seen = DT.trace "HIT1" $ destruct ((q,(mkUni u, mkArr p a)) :lqc) table (c:seen)
destruct (c@(QL l q,((In (Inr (Uni u))),a)):lqc) table seen = DT.trace "HIT2" $ destruct ((q,(mkUni u, mkSRecT l a)) :lqc) table (c:seen)
...
instFreeVars :: [TyName] -> Subst'
instFreeVars []     = []
instFreeVars (t:ts) = (PosT, translate t, mkTopT):(NegT, translate t, mkTopT):(instFreeVars ts)
