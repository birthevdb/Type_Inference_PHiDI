...
instance FPretty I.FType where
  ppr (I.Arr t1 t2) = do
    ...
    t2' <- ppr t2
    return $ Pretty.parens (t1' <+> "→" <+> t2')
  ppr I.NumT = return $ "Int"
  ppr I.BoolT = return $ "Bool"
  ppr (I.And t1 t2) = do
    t1' <- ppr t1
    t2' <- ppr t2
    return $ Pretty.parens (t1' <+> "&" <+> t2')
  ppr (I.TVar x) = return (Pretty.pretty x)
  ppr (I.SRecT l t) = do
    t' <- ppr t
    return (Pretty.braces $ Pretty.pretty l <+> Pretty.colon <+> t')
  ppr I.TopT = return $ "Top"
  ppr I.BotT = return $ "Bottom"
  ppr (I.OpAbs b) =
    lunbind b $ \((x, Embed k), t) -> do
      ...
      return $
        Pretty.parens
          ("Lambda" <> Pretty.parens (Pretty.pretty x <> ":" <> k') <+>
           Pretty.dot <+> t')
  ppr (I.OpApp a b) = do
    a' <- ppr a
    b' <- ppr b
...
wrapargi :: I.FExpr -> FDoc -> FDoc
wrapargi x = case x of
  I.Var _   -> identity
  I.LitV _  -> identity
  I.BoolV _ -> identity
  I.Top     -> identity
...
instance FPretty I.FExpr where
  ppr (I.Anno e t) = do
    e' <- ppr e
    t' <- ppr t
    return $ enclose' "" "" " : " ": " (fmap duplicate ([e', t']))
  ppr (I.Var x) = return $ Pretty.pretty x
  ppr (I.App f a) = do
    df <- ppr f
    da <- ppr a
    return $ wrapfi f df <+> wrapargi a da
  ppr (I.TApp f a) = do
    f' <- ppr f
    a' <- ppr a
    return $ wrapfi f f' <+> ("[" <> a' <> "]")
  ...
  ppr (I.LitV n) = return (Pretty.pretty n)
  ppr (I.BoolV b) = return (Pretty.pretty b)
  ppr (I.PrimOp op e1 e2) = do
    e1' <- ppr e1
    op' <- ppr op
    e2' <- ppr e2
    return $
  ...
  ppr (I.Letrec b) =
    lunbind b $ \((x, Embed t), (e, body)) -> do
      ...
      let d = [Pretty.group (Pretty.flatAlt long short), b']
      return $ enclose' "" "" " in " ("in  ") (fmap duplicate d)
  ppr I.Bot = return $ "undefined"
  ppr (I.DRec' _) = return $ "fancy records"
  ppr (I.Pos _ e) = ppr e
